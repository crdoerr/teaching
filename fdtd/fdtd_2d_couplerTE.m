...
nsi = 2.84;           % TE effective index of silicon
...
ya = (1:Ny)*Dy;
envy = 50*Dx*exp(-((ya-offset-ylim)/wg.w*2).^2)/2;
%% initialize fields
Ex = zeros(Nx,Ny);
Ey = zeros(Nx,Ny);
Ez = zeros(Nx,Ny);
...
for cnt = 1:numsteps
    t = cnt*Dt;
    %% inject field
    Hz(2,1:Ny) = Hz(2,1:Ny) + envy.*cos(2*pi*fc*t);  %*exp(-((t-pulsew*3)/pulsew)^2);
    %% calculate H field
    Hznew = Hz + Dtu0Dy*(circshift(Ex,[0 -1]) - Ex) - Dtu0Dx*(circshift(Ey,[-1 0]) - Ey);
    Hznew(Nx,:) = Ey(Nx,:)./sqrt(u0/e0./ery(Nx,:));
    Hznew(:,Ny) = -Ex(:,Ny)./sqrt(u0/e0./ery(:,Ny));
    Hz = Hznew;
    %% calculate E field
    Exnew = Ex + Dte0x.*(Hz - circshift(Hz,[0 1]))/Dy;
    Eynew = Ey - Dte0y.*(Hz - circshift(Hz,[1 0]))/Dx;
    Eynew(1,:) = -Hz(1,:).*sqrt(u0/e0./ery(1,:));
    Exnew(:,1) = Hz(:,1).*sqrt(u0/e0./erx(:,1));
    Ex = Exnew;
    Ey = Eynew;
    %% plot field
    if round(cnt/Nplot)==cnt/Nplot
        h = surf(x,y,Hz.'+ erplot);
